...
def serialize_ticket(t):
    ...
    return {
        ...
        "reparto": t.reparto,
        "note_assegnazione": t.note_assegnazione or "",
        "esecutore": {
            "id": t.esecutore_id,
            "username": t.esecutore.username,
            "nome_completo": t.esecutore.nome_completo,
        },
        "revisore": {
            "id": t.revisore_id,
            "username": t.revisore.username,
            "nome_completo": t.revisore.nome_completo,
        },
        "approvatore": {
            "id": t.approvatore_id,
            "username": t.approvatore.username,
            "nome_completo": t.approvatore.nome_completo,
        },
        "assegnato_da": {"id": t.assegnato_da_id, "nome_completo": t.assegnato_da.nome_completo}
        if t.assegnato_da_id
        else None,
...
def _serialize_ticket_revisione(r):
    ...
    # Use client feedback from this revision; if absent and this was created after
    # a client response (crea_nuova_rev=True), fall back to the previous revision.
    ext_status_label = r.ext_status.nome if r.ext_status else ""
    ext_status_colore = r.ext_status.colore if r.ext_status else ""
    note_rientro = r.note_rientro or ""
    from_prev_rev = False
    if r.crea_nuova_rev and not ext_status_label and not note_rientro:
        prev = (
            r.documento.revisioni.filter(rev_no__lt=r.rev_no)
            .order_by("-rev_no")
            .select_related("ext_status")
            .first()
        )
        if prev:
            ext_status_label = prev.ext_status.nome if prev.ext_status else ""
            ext_status_colore = prev.ext_status.colore if prev.ext_status else ""
            note_rientro = prev.note_rientro or ""
            from_prev_rev = bool(ext_status_label or note_rientro)
    ...
    return {
        ...
        "ext_status_label": ext_status_label,
        "ext_status_colore": ext_status_colore,
        "reparto_nome": r.documento.reparto or "",
        "note_rientro": note_rientro,
        "from_prev_rev": from_prev_rev,
    }
...
def serialize_notifica(n):
    ...
    mittente_info = None
    if n.mittente_id and n.mittente:
        mittente_info = {"id": n.mittente_id, "nome_completo": n.mittente.nome_completo}
    return {
        "id": n.pk,
        "testo": n.testo,
        "ticket_id": n.ticket_id,
        "ticket": ticket_info,
        "mittente": mittente_info,
        "letta": n.letta,
        "created_at": n.created_at.isoformat() if n.created_at else None,
    }
...
def list_tickets(reparto=None, concluso=None, commessa=None):
    ...
    """
    qs = Ticket.objects.select_related("esecutore", "revisore", "approvatore").prefetch_related(
        "revisioni"
    )
    if reparto:
        qs = qs.filter(reparto=reparto)
    if commessa:
        qs = qs.filter(revisioni__documento__testata__job__icontains=commessa).distinct()
...
def get_ticket(pk):
    return Ticket.objects.select_related("esecutore", "revisore", "approvatore").get(pk=pk)
...
def create_ticket(data, user):
    ...
    t = Ticket(
        ...
        note_assegnazione=data.get("note_assegnazione") or "",
    )
    t.full_clean()
    t.save()
    ...
    if revisione_ids:
        ...
        update_kwargs = {"int_status": "da_iniziare"}
        if data.get("dis_plan_date"):
            from datetime import date as _date

            try:
                update_kwargs["dis_plan_date"] = _date.fromisoformat(data["dis_plan_date"])
            except (ValueError, TypeError):
                pass
        Revisione.objects.filter(id__in=revisione_ids).update(**update_kwargs)
    ...
    for uid in destinatari:
        Notifica.objects.create(
            destinatario_id=uid,
            mittente=user,
            testo=f"ha assegnato il Ticket {t.nome} ({t.reparto})",
            ticket=t,
        )
...
@transaction.atomic
def update_ticket(pk, data, user=None):
    t = Ticket.objects.get(pk=pk)
    old_assignees = {t.esecutore_id, t.revisore_id, t.approvatore_id}
    for field in ("reparto", "esecutore_id", "revisore_id", "approvatore_id", "note_assegnazione"):
        if field in data:
    ...
    if "revisione_ids" in data:
        ...
        if removed:
            Revisione.objects.filter(id__in=removed, int_status__in=STATI_ATTIVI_REV).update(
                int_status=""
            )
    t.full_clean()
    t.save()

    # Notify all current assignees about the update
    new_assignees = {t.esecutore_id, t.revisore_id, t.approvatore_id}
    destinatari = (old_assignees | new_assignees) - {None}
    for uid in destinatari:
        Notifica.objects.create(
            destinatario_id=uid,
            mittente=user,
            testo=f"ha aggiornato il Ticket {t.nome} ({t.reparto})",
            ticket=t,
        )
...
def transition_revisione(rev_pk, action, user, nota_testo=None):
    ...
    ticket = (
        rev.tickets.select_related("esecutore", "revisore", "approvatore")
        .order_by("-created_at")
        .first()
    )
    if ticket is None:
    ...
    _send_rev_transition_notifications(ticket, rev, action, new_status, user=user)

    return ticket, rev


def set_revisione_status(pk, new_status, user=None):
    """
    Directly set int_status on a Revisione, bypassing the transition permission system.
    If new_status is '' (empty), the revision is detached from any active ticket and
    returned to the 'non_assegnato' pool.
    """
    valid = {"", "da_iniziare", "in_lavorazione", "in_revisione", "in_approvazione", "da_emettere"}
    if new_status not in valid:
        raise ValueError(f"Stato '{new_status}' non valido.")

    with transaction.atomic():
        rev = Revisione.objects.select_related("documento").get(pk=pk)

        if new_status == "":
            # Detach from any active ticket
            for ticket in rev.tickets.all():
                ticket.revisioni.remove(rev)
            rev.int_status = ""
        else:
            old_status = rev.int_status or ""
            rev.int_status = new_status
            # Send notifications based on destination status
            ticket = (
                rev.tickets.select_related("esecutore", "revisore", "approvatore")
                .order_by("-created_at")
                .first()
            )
            if ticket and new_status != old_status:
                _send_drag_notifications(ticket, rev, new_status, user=user)

        rev.save(update_fields=["int_status"])
        return rev


def _send_drag_notifications(ticket, rev, new_status, user=None):
    """Send notifications when a card is dragged to a new column."""
    rev_label = f"{rev.documento.vendor_doc or rev.documento.doc_title or 'Doc'} rev {rev.rev_no}"
    notifiche = []  # list of (user_id, testo)

    if new_status == "in_revisione":
        notifiche.append(
            (ticket.revisore_id, f"ha spostato {rev_label} in revisione nel Ticket {ticket.nome}")
        )
    elif new_status == "in_approvazione":
        notifiche.append(
            (
                ticket.approvatore_id,
                f"ha spostato {rev_label} in approvazione nel Ticket {ticket.nome}",
            )
        )
    elif new_status == "da_emettere":
        destinatari = {ticket.esecutore_id, ticket.revisore_id, ticket.approvatore_id} - {None}
        for uid in destinatari:
            notifiche.append(
                (uid, f"ha spostato {rev_label} in Da emettere nel Ticket {ticket.nome}")
            )
    elif new_status == "in_lavorazione":
        notifiche.append(
            (
                ticket.esecutore_id,
                f"ha spostato {rev_label} di nuovo in lavorazione nel Ticket {ticket.nome}",
            )
        )

    # Exclude the user doing the action from their own notifications
...
def _send_rev_transition_notifications(ticket, rev, action, new_status, user=None):
    notifiche = []
...
def list_revisioni_da_emettere(reparto=None, commessa=None):
    """
    Return revisions that are pending ticket assignment:
    1. Rev 0 of documents that don't yet have a ticket
    2. Revisions with crea_nuova_rev=True (returned from client) without a ticket
    A revision is "unassigned" when its int_status is empty.
    """
...
def count_tickets_attivi_per_reparto():
    ...
    qs = (
        Ticket.objects.filter(revisioni__int_status__in=STATI_ATTIVI_REV)
        .values("reparto")
...
def revisioni_preview_for_commessa(job):
    """
    Returns Kanban distribution and top urgent items for the commessa preview card.
    Counts per column + overdue count + top 3 items by dis_plan_date.
    """
    from datetime import date as _date
    ...
    # Non-assegnato: revisions with int_status='' for this commessa
    non_ass_qs = Revisione.objects.filter(
        Q(rev_no=0, int_status="") | Q(crea_nuova_rev=True, int_status=""),
        documento__testata__job=job,
    ).select_related("documento")
    counts["non_assegnato"] = non_ass_qs.count()

    # Workflow columns: from tickets for this commessa
    workflow_qs = Revisione.objects.filter(
        tickets__commessa=job,
        int_status__in=[
            "da_iniziare",
            "in_lavorazione",
            "in_revisione",
    ...
    # Overdue: revisions with dis_plan_date < today (any active column)
    overdue = Revisione.objects.filter(
        Q(
            Q(rev_no=0, int_status="")
            | Q(crea_nuova_rev=True, int_status="")
            | Q(
                int_status__in=[
                    "da_iniziare",
                    "in_lavorazione",
        ...
        documento__testata__job=job,
        dis_plan_date__lt=today,
    ).count()

    # Top 3 most urgent items (nearest dis_plan_date, non-null)
    def _urgency(delta):
        if delta is None:
            return "urg-none", "—"
        if delta < -1:
    ...
    urgent_qs = (
        Revisione.objects.filter(
            ...
            documento__testata__job=job,
            dis_plan_date__isnull=False,
        )
        .select_related("documento")
        .order_by("dis_plan_date")[:3]
    )

    top_items = []
    for rev in urgent_qs:
        delta = (rev.dis_plan_date - today).days if rev.dis_plan_date else None
        urg_class, days_label = _urgency(delta)
        top_items.append(
            {
                "vendor_doc": rev.documento.vendor_doc or rev.documento.doc_title,
                "rev_no": rev.rev_no,
...
def list_note(ticket_pk):
    return [
        serialize_nota(n)
        for n in TicketNota.objects.filter(ticket_id=ticket_pk).select_related("autore")
    ]
...
def list_notifiche(user, solo_non_lette=False):
    qs = Notifica.objects.filter(destinatario=user).select_related("ticket", "mittente")
    if solo_non_lette:
...
def get_user_overview(user):
    ...
    base = (
        Ticket.objects.select_related("esecutore", "revisore", "approvatore")
